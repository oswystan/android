...
#define CHECK_ARG(arg, cnt) \
    do {\
        if ( arg.size() != cnt ) {\
            loge("invalid argument size %u", arg.size());\
            return -1;\
        }\
    }while(0)
...
public:
    ...
    void postDataTimestamp(nsecs_t timestamp, int32_t msgType, const sp<IMemory>& dataPtr) {
        ...
        frame_cnt++;
        logd("ts gap = %ld cur_ts=%ld", cur_ts - pre_ts, cur_ts);
        pre_ts = cur_ts;
        ...
        last_ts = get_cur_ms();
        logd("delta=%lu", last_ts - cur_ts);

        ssize_t offset;
        size_t size;
        sp<IMemoryHeap> heap = dataPtr->getMemory(&offset, &size);

        logd("heap fd=%d offset=%ld size=%u", heap->getHeapID(), offset, size);
        void *heapBase = dataPtr->pointer();
        logd("heap size: %d", dataPtr->size());
        void* ptr = NULL;
        int jpeg_size = 0;
        unsigned long j1 = get_cur_ms();
        int ret = jpegenc_encode(heapBase, heap->getHeapID(), &ptr, &jpeg_size);
        unsigned long j2 = get_cur_ms();
        logd("jpeg ret = %d size=%d delta=%lu", ret, jpeg_size, j2-j1);
        if (frame_cnt < 10) {
            char fn[128];
            sprintf(fn, "/data/misc/camera/%02lu.jpeg", frame_cnt);
            FILE* fp = fopen(fn, "wb");
            if (fp) {
                fwrite(ptr, 1, jpeg_size, fp);
                fclose(fp);
            }
        }
...
int cmd_set_parameter(stc_t* stc, std::vector<std::string>& arg) {
    ...
    camPara.set(CameraParameters::KEY_RECORDING_HINT, "1");
...
int cmd_start_recording(stc_t* stc, std::vector<std::string>& arg) {
    ...
    //cam->storeMetaDataInBuffers(false);
    int ret = cam->startRecording();
    if (ret != 0) {
        loge("fail to start recording");
        return ret;
    }
    jpegenc_init(c->videoWidth, c->videoHeight);
    return 0;
}
